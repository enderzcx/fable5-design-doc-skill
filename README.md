...
Fable5 Design Doc Skill 是一个公共安全的提示词打包技能。它帮助你为 Fable5，或其他 AI 设计代理，创建可复用的提示文档和简短启动提示，用来产出带预览图参照的设计文档。该技能包含强制澄清门、视觉预览产物要求、质量检查器和可复用模板。它面向开源使用，不包含私有路径、项目、凭据或个人偏好。
...
- 你希望 Fable5 产出预览图、截图或渲染 mock，供后续实现参照。
...
3. 要求 Fable5 产出可打开、可引用的视觉预览产物。
4. 使用 `templates/fable5-design-prompt.md` 创建提示文档。
5. 使用 `templates/launch-prompt.md` 创建简短启动提示。
6. 运行 `scripts/check_fable5_design_prompt.py` 验证生成的提示文档。
...
4. **创建提示词包** - 使用 `templates/fable5-design-prompt.md` 和 `templates/launch-prompt.md`，并写入视觉预览产物要求。
...
当必需章节缺失、视觉预览章节缺失或模板占位符未替换时，检查器会失败。使用提示词前请修复所有问题。
